...
module tetris(
    ...
    reg  [`BITS_PER_BLOCK-1:0] next_piece; 
    ...
    wire [`BITS_BLK_POS-1:0] next_blk_1;  //change yhl -------------------------------
    ...
    vga_display display_ (
        ...
        .HSY(HSY),
        .VSY(VSY),
        .PIC_display_off(PIC_display_off)
    );
    ...
    //change yhl ----------------------------------------------------------------------
   cal_next_blk cal_next_test_blk(
     .piece(next_piece),
    .blk_1(next_blk_1),
    .blk_2(next_blk_2),
    .blk_3(next_blk_3),
    ...
    // fallen blocks.
    wire test_intersects = intersects_fallen_pieces(test_blk_1, test_blk_2, test_blk_3, test_blk_4);    //判定重叠信号的产生条件
    ...
    wire game_over = cur_pos_y == 0 && intersects_fallen_pieces(cur_blk_1, cur_blk_2, cur_blk_3, cur_blk_4);    //game over判定
    ...
    always @ (posedge clk) begin
        ...
        game_clk_rst <= 0;
        if (mode == `MODE_IDLE && (sw_rst_en == 1)&&( sw_rst_dis == 0)) begin     //20190724ZPJ:修改成只有sw_rst从0到1才开始游戏
...
reg end_signal = 0;
always@(posedge clk_too_fast)
begin
    if(game_over == 1)
        end_signal <= 1;
    else if(sw_rst == 0)
        end_signal <= 0;
    else end_signal <= end_signal;
end

//ZPJ: Display picture before game start and after game over.
display_picture_SO diaplay_pictureso(
//system signals
.CLK_100MHZ_i(clk_too_fast),
.start(sw_rst),
.over(end_signal),
//vga signals
.vga_hs_o(HSY),
